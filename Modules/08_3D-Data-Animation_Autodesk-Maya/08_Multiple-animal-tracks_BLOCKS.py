...
PINKcolor = [246/255, 152/255, 152/255] # pink
...
mtl_PINK, PINK = create_shader("PINK")
cmds.setAttr(mtl_PINK + ".color", PINKcolor[0], PINKcolor[1], PINKcolor[2], type="double3")
# ---------------------------------------------------------------------------------------
...
result = cmds.polyCube (w=2, h=0.5, d=1, name='myCube#') # d = e seal width
...
with open('G:/My Drive/Visualization/Data/08_Bubble-Nets_LBAS_01_ALLWHALES_1Hz.csv') as trackcsv_file:
    ...
    for row in trackreader: #for row in reader:
        ...
        instanceResult = cmds.instance(transformName, name=transformName + '_instance%s' % row_count)
        cmds.parent(instanceResult, instanceGroupName) # Parent all of your cubes into the instance group
        cmds.setAttr(transformName + '_instance%s' % row_count + '.rotateOrder', 3, instanceResult)
        ...
        yRot = float(row['head']) * (180/math.pi) -90 # NOT exactly heading because heading is the direction of your pitched and rolled object (not the z rotation)
        ...
        if whaleName == 'mn210614-55':
            cmds.sets(instanceResult, forceElement=SWS)
        elif whaleName == 'mn210614-58':
            cmds.sets(instanceResult, forceElement=AW)
        elif whaleName == 'mn210614-95':
            cmds.sets(instanceResult, forceElement=QW)
        elif whaleName == 'mn210614-97':
            cmds.sets(instanceResult, forceElement=PINK)
        else:
            cmds.sets(instanceResult, forceElement=REM)
        # --------------------------------------------------------------------------

        #if row_count == 10000: # STOP AT ROW 100
        #    break
